...
class TestPlace < Test::Unit::TestCase
	...
	def test_new
		...
		assert_raise(ArgumentError) {Place.new(0,0,0,0)}
		assert_raise(ArgumentError) {Place.new(1,0,10,0)}
		assert_raise(ArgumentError) {Place.new(1,0,0,20)}
		assert_raise(ArgumentError) {Place.new(-1,0,10,20)}
		assert_raise(ArgumentError) {Place.new(1,-2,10,20)}
		assert_raise(ArgumentError) {Place.new(1,0,-10,20)}
		assert_raise(ArgumentError) {Place.new(1,0,10,-20)}
		assert_raise(ArgumentError) {Place.new(-1,0,-10,-20)}
	end
